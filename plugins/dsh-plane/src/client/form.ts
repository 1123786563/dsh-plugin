/**
 * Staged-form model behind the plane settings card. A card stages what the
...
export interface PlaneScopeSnapshot {
...
export interface PlaneCardState {
...
export interface PlaneCardFace {
  hooks: {
    /** Card snapshot, bound by the renderer as usePlaneSettingsCard. */
    planeSettingsCard: PlaneCardStore
  }
...
export interface PlaneCardStore {
  getSnapshot(): PlaneCardState
  subscribe(listener: () => void): () => void
}
...
export class PlaneSettingsCardController {
  private readonly scope: PlaneSettingsScope
  private readonly staged = new Map<PlaneField, Staged>()
  private readonly listeners = new Set<() => void>()
  private saving = false
  private failed = false
  private failedReason: string | undefined
  private state: PlaneCardState
  private readonly unsubscribe: () => void

  /**
   * Bind the namespace scope and derive the first card state.
   * @param scope - the bound settings scope for the plane namespace.
   */
  constructor(scope: PlaneSettingsScope) {
    this.scope = scope
   ...
   */
  inject(): PlaneCardFace {
    return {
      hooks: { planeSettingsCard: this.store() },
  ...
  async save(): Promise<void> {
    ...
    try {
      for (const [field, staged] of [...this.staged]) {
        ...
        if (field === 'apiKey' && staged.text.length === 0) continue // redacted secret: empty draft means no change
        if (field === 'perPage') {
          await this.scope.set(field, Number.parseInt(staged.text, 10))
        } else {
   ...
   */
  private stored(field: PlaneField): string {
   ...
   */
  private derive(): PlaneCardState {
    const snapshot = this.scope.getSnapshot()
    const available = snapshot.status !== 'loading' && snapshot.value !== undefined
    const exposed = snapshot.status !== 'unavailable'
    const fields = {} as Record<PlaneField, PlaneFieldState>
    let invalid = false
    for (const field of [...TEXT_FIELDS, 'perPage'] as const) {
